...
<!-- Toolbar Filters -->
<div class="toolbar" style="display: flex; flex-direction: column; gap: var(--space-4); margin-bottom: var(--space-4); background: var(--bg-surface); padding: var(--space-4); border-radius: 12px; border: 1px solid var(--gray-200);">
  ...
  <div style="display: flex; flex-wrap: wrap; gap: var(--space-4); align-items: center; width: 100%;">
    ...
    <div style="display: flex; gap: var(--space-2); align-items: center; min-width: 220px;">
      <span style="font-size: var(--text-xs); font-weight: 700; color: var(--gray-500); text-transform: uppercase;">Ciclo:</span>
      <select class="form-control form-select" onchange="setCycleAdmin(this.value)" style="border-radius: 20px; font-weight: 600; height: 36px; padding: 0 var(--space-4);">
    ...
    <div style="display: flex; gap: var(--space-2); align-items: center; min-width: 280px; flex: 1;">
      <span style="font-size: var(--text-xs); font-weight: 700; color: var(--gray-500); text-transform: uppercase;">Grupo:</span>
      <select class="form-control form-select" onchange="setGroupAdmin(this.value)" style="border-radius: 20px; font-weight: 600; height: 36px; padding: 0 var(--space-4);">
  ...
  <div style="display: flex; gap: var(--space-3); width: 100%; border-top: 1px solid var(--gray-100); padding-top: var(--space-3); align-items: center;">
    <!-- 4. Search Input -->
    <div style="position: relative; flex: 1;">
      <input type="text" id="search-input" class="form-control" placeholder="Buscar alumnos por matrícula o nombre..." 
             value="<?= htmlspecialchars($searchQuery) ?>" 
             style="height: 38px; padding-left: var(--space-10); border-radius: 20px;"
             onkeypress="checkSearch(event)">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" style="position: absolute; left: var(--space-4); top: 11px; width: 16px; height: 16px; color: var(--gray-400);">
        <circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/>
      </svg>
    </div>
    <button class="btn btn-outline" onclick="applyFilters()" style="height: 38px; border-radius: 20px; padding: 0 var(--space-6);">Buscar</button>
    <?php if (!empty($searchQuery) || $selectedGroupId !== 'all'): ?>
      <button class="btn btn-outline" onclick="clearFilters()" style="height: 38px; border-radius: 20px; color: var(--gray-500);">Limpiar</button>
    <?php endif; ?>
  </div>
    ...
    <tbody>
      ...
      <?php else: ?>
        ...
        <tr id="row-alumno-<?= $alumno['id_alumno'] ?>">
          <td class="col-matricula" style="font-weight: 700; color: var(--primary-800);"><?= htmlspecialchars($alumno['matricula']) ?></td>
          <td class="col-name" style="font-weight: 600;"><?= htmlspecialchars($alumno['nombre_completo']) ?></td>
          <td class="col-career">
            <span class="badge badge-outline" style="font-weight: 700; color: var(--uts-green); border-color: var(--uts-green);">
              <?= htmlspecialchars($alumno['siglas'] . $alumno['cuatrimestre'] . $alumno['grupo']) ?>
            </span>
            <span style="font-size: var(--text-xs); color: var(--gray-500); margin-left: var(--space-2);"><?= htmlspecialchars($alumno['carrera']) ?></span>
          </td>
          <td style="text-align: center;">
            <div style="display:inline-flex; gap: var(--space-2);">
              <button class="btn btn-outline btn-sm" onclick="editStudentAdmin(<?= htmlspecialchars(json_encode($alumno)) ?>)" title="Editar">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14">
                  <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                  <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
                </svg>
              </button>
              <button class="btn btn-outline btn-sm" onclick="deleteStudentAdmin(<?= $alumno['id_alumno'] ?>, <?= $alumno['id_grupo'] ?>, '<?= htmlspecialchars($alumno['nombre_completo']) ?>')" title="Eliminar del Grupo">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="14" height="14">
...
<!-- Pagination Controls -->
<?php if ($totalPages > 1): ?>
<div class="pagination" style="display: flex; justify-content: center; align-items: center; gap: var(--space-2); margin-top: var(--space-6);">
  <button class="btn btn-outline btn-sm" <?= $pageCurrent === 1 ? 'disabled' : '' ?> onclick="changePage(<?= $pageCurrent - 1 ?>)">« Anterior</button>
  
  <?php for ($p = 1; $p <= $totalPages; $p++): ?>
    <button class="btn <?= $p === $pageCurrent ? 'btn-primary' : 'btn-outline' ?> btn-sm" style="min-width: 32px; height: 32px;" onclick="changePage(<?= $p ?>)"><?= $p ?></button>
  <?php endfor; ?>
  
  <button class="btn btn-outline btn-sm" <?= $pageCurrent === $totalPages ? 'disabled' : '' ?> onclick="changePage(<?= $pageCurrent + 1 ?>)">Siguiente »</button>
</div>
<?php endif; ?>
  ...
  <div class="modal">
    ...
    <form id="form-student-admin" onsubmit="saveStudentAdmin(event)">
      ...
      <div class="form-group" id="group-select-container">
        ...
        <p style="font-size: 11px; color: var(--gray-500); margin-top: 4px;">El alumno se inscribirá automáticamente en este grupo.</p>
      </div>

      <div class="form-group">
        <label class="form-label" for="student-matricula-admin">Matrícula</label>
        <input type="text" class="form-control" id="student-matricula-admin" name="matricula" placeholder="Ej. 612310503" required>
      </div>
      ...
      <div style="display: flex; gap: var(--space-3); margin-top: var(--space-6);">
        <button type="button" class="btn btn-outline" style="flex:1;" onclick="closeModal('modal-student-admin')">Cancelar</button>
